...
const LAYOUT_CONFIG = {
    ...
    PRO_CON_VERTICAL_SPACING: 25,
    MIN_DISTANCE_FROM_PARENT: 40,
    CANVAS_WIDTH: 5000,
    CANVAS_HEIGHT: 5000
};
...
function createNode(x = 100, y = 200, text = 'Новый исход', parentId = null, type = null) {
    ...
    let freePos = findFreePosition(targetX, targetY, parentId, 0);
...
function makeDraggable(elem) {
    ...
    const isProCon = nodeType.includes('pro') || nodeType.includes('con');
...
function fixProConPositions() {
    const proConNodes = Array.from(document.querySelectorAll('.node[data-id]'))
        .filter(n => {
            ...
            return nodeType && (nodeType === 'pro' || nodeType === 'con' ||
    ...
    if (!renderConnections._frame) {
        renderConnections._frame = requestAnimationFrame(() => {
            ...
            edges.forEach(e => {
                ...
                const isProConConnection = isProConnection || isConConnection;
                ...
                controlOffset = Math.min(Math.max(horizontalDistance * 0.4, verticalDistance * 0.3), 100);

                if (isProConConnection && verticalDistance > horizontalDistance) {
                    controlOffset = Math.min(horizontalDistance * 0.2, 40);
                }
...
function createProConBranches(outcomeNode, pros, cons, outcomeIndex) {
    ...
    // Собираем все существующие блоки плюсов/минусов для этого outcome
    const existingProConRects = [];
    const allProConNodes = Array.from(document.querySelectorAll('.node[data-id]')).filter(n => {
        const nodeType = n.dataset.type;
        return nodeType && (nodeType.includes('pro') || nodeType.includes('con'));
    });

    // Находим родителей этих блоков
    allProConNodes.forEach(node => {
        const parentEdge = edges.find(e => e.to === node.dataset.id);
        if (parentEdge) {
            const parentNode = byId(parentEdge.from);
            if (parentNode && parentNode !== outcomeNode) {
                const rect = occupiedRectangles.get(node.dataset.id);
                if (rect) {
                    existingProConRects.push(rect);
                }
            }
        }
    });
    ...
    function isPositionAvailable(x, y, width, height, excludeId = null) {
        // Проверяем границы
        if (x < BOUNDARIES.minX || y < BOUNDARIES.minY) return false;
        if (x + width > BOUNDARIES.maxX || y + height > BOUNDARIES.maxY) return false;

        // Проверяем, что не выше родителя
        if (y < outcomeRect.bottom + LAYOUT_CONFIG.MIN_DISTANCE_FROM_PARENT) return false;
        ...
        for (const rect of existingProConRects) {
            if (rectanglesIntersect(testRect, rect, 10)) {
                return false;
            }
        }
    ...
    // Функция для поиска свободной позиции
    function findAvailablePosition(startX, startY, width, height) {
        let currentY = startY;
        const MAX_ATTEMPTS = 30;

        for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
            if (isPositionAvailable(startX, currentY, width, height)) {
                return { x: startX, y: currentY };
            }
            // Двигаемся вниз
            currentY += height + LAYOUT_CONFIG.PRO_CON_VERTICAL_SPACING;
        }

        // Если не нашли, возвращаем последнюю проверенную позицию
        return { x: startX, y: currentY };
    }

    // Создаем блоки плюсов
    if (pros.length > 0) {
        const proContainerX = outcomeRect.x - LAYOUT_CONFIG.HORIZONTAL_OFFSET;
        const proContainerY = outcomeRect.bottom + LAYOUT_CONFIG.MIN_DISTANCE_FROM_PARENT;

        const clampedProX = Math.max(BOUNDARIES.minX, proContainerX);
        let clampedProY = Math.max(proContainerY, outcomeRect.bottom + LAYOUT_CONFIG.MIN_DISTANCE_FROM_PARENT);

        // Ищем свободное место для контейнера плюсов
        let containerFound = false;
        for (let offsetY = 0; offsetY < 10; offsetY++) {
            const testY = clampedProY + offsetY * 50;
            if (isPositionAvailable(clampedProX, testY, LAYOUT_CONFIG.NODE_WIDTH, LAYOUT_CONFIG.NODE_HEIGHT)) {
                clampedProY = testY;
                containerFound = true;
                break;
            }
        ...
        if (!containerFound) {
            clampedProY = outcomeRect.bottom + LAYOUT_CONFIG.MIN_DISTANCE_FROM_PARENT;
        }
        ...
        if (prosContainer) {
            ...
            // Создаем отдельные плюсы
            let currentProY = clampedProY + LAYOUT_CONFIG.NODE_HEIGHT + LAYOUT_CONFIG.PRO_CON_VERTICAL_SPACING;

            pros.forEach((pro, idx) => {
                const pos = findAvailablePosition(clampedProX, currentProY,
                    LAYOUT_CONFIG.NODE_WIDTH, LAYOUT_CONFIG.PRO_CON_ITEM_HEIGHT);

                const proId = createNode(pos.x, pos.y, pro, prosContainerId, 'pro');

                // Добавляем созданный блок в список для проверки
                const createdNode = byId(proId);
                if (createdNode) {
                    setTimeout(() => {
                        const rect = occupiedRectangles.get(proId);
                        if (rect) {
                            existingProConRects.push(rect);
                        }
                    }, 50);
                ...
                currentProY = pos.y + LAYOUT_CONFIG.PRO_CON_ITEM_HEIGHT + LAYOUT_CONFIG.PRO_CON_VERTICAL_SPACING;
            });
    ...
    // Создаем блоки минусов
    if (cons.length > 0) {
        const conContainerX = outcomeRect.x + LAYOUT_CONFIG.HORIZONTAL_OFFSET;
        const conContainerY = outcomeRect.bottom + LAYOUT_CONFIG.MIN_DISTANCE_FROM_PARENT;

        const clampedConX = Math.min(conContainerX, BOUNDARIES.maxX - LAYOUT_CONFIG.NODE_WIDTH);
        let clampedConY = Math.max(conContainerY, outcomeRect.bottom + LAYOUT_CONFIG.MIN_DISTANCE_FROM_PARENT);

        // Ищем свободное место для контейнера минусов
        let containerFound = false;
        for (let offsetY = 0; offsetY < 10; offsetY++) {
            const testY = clampedConY + offsetY * 50;
            if (isPositionAvailable(clampedConX, testY, LAYOUT_CONFIG.NODE_WIDTH, LAYOUT_CONFIG.NODE_HEIGHT)) {
                clampedConY = testY;
                containerFound = true;
                break;
            }
        ...
        if (!containerFound) {
            clampedConY = outcomeRect.bottom + LAYOUT_CONFIG.MIN_DISTANCE_FROM_PARENT;
        }
        ...
        if (consContainer) {
            ...
            // Создаем отдельные минусы
            let currentConY = clampedConY + LAYOUT_CONFIG.NODE_HEIGHT + LAYOUT_CONFIG.PRO_CON_VERTICAL_SPACING;

            cons.forEach((con, idx) => {
                const pos = findAvailablePosition(clampedConX, currentConY,
                    LAYOUT_CONFIG.NODE_WIDTH, LAYOUT_CONFIG.PRO_CON_ITEM_HEIGHT);

                const conId = createNode(pos.x, pos.y, con, consContainerId, 'con');

                // Добавляем созданный блок в список для проверки
                const createdNode = byId(conId);
                if (createdNode) {
                    setTimeout(() => {
                        const rect = occupiedRectangles.get(conId);
                        if (rect) {
                            existingProConRects.push(rect);
                        }
                    }, 50);
                ...
                // Обновляем Y для следующего блока
                currentConY = pos.y + LAYOUT_CONFIG.PRO_CON_ITEM_HEIGHT + LAYOUT_CONFIG.PRO_CON_VERTICAL_SPACING;
            });
